...
//        else if(!tank_en) current_state <= WAIT;
    ...
    always @(*) begin
        case(bullet_direction[1:0])
            ...
            2'b10: begin
                bullet_bound <= (bullet_H_feedback <= vgaH && bullet_H_feedback > vgaH - BULLET_LONGER && bullet_V_feedback > vgaV - BULLET_SHORTER) ? 1 : 0;
            end
            2'b11: begin
    ...
    tank_data_selector bullet_selector(.clk(clk), .UP(UP_data), .DOWN(DOWN_data), .LEFT(LEFT_data), 
                            .RIGHT(RIGHT_data), .Dir(bullet_dir_feedback), .tankData(outData));
